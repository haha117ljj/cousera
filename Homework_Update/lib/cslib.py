...
"""
Created on Wed Jul 15 22:47:18 2020
...
def convert_to_ts(df_orig, country=None):
  ...
  start_month = '{}-{}'.format(
    df['year'].values[0],
    str(df['month'].values[0]).zfill(2)
  )
  stop_month = '{}-{}'.format(
    df['year'].values[-1],
    str(df['month'].values[-1]).zfill(2)
  )
  ...
  invoices = [
    np.unique(df[df_dates == day]['invoice'].values).size for day in days
  ]
  streams = [
    np.unique(df[df_dates == day]['stream_id'].values).size for day in days
  ]
...
def fetch_ts(data_dir, clean=False):
  ...
  if len(os.listdir(ts_data_dir)) > 0:
    ...
    return (
      {
        re.sub(r"\.csv", "", cf)[3:]:
        pd.read_csv(os.path.join(ts_data_dir, cf))
...
def engineer_features(df, training=True):
  ...
  for d, day in enumerate(dates):
    ...
    mask = np.in1d(
      dates, np.arange(start_date, stop_date, dtype='datetime64[D]')
    )

...
from logger1 import update_predict_log, update_train_log
...
def _model_train(df,tag,test=False):
    ...
    pipe_adb = Pipeline(steps=[('scaler', StandardScaler()),
    ...
    update_train_log(tag,X.shape,eval_rmse,runtime,
...
def model_load(prefix='sl',data_dir=None,training=True):
    ...
    if not data_dir:
        data_dir = os.path.join("train_data","cs-train")
    
    models = [f for f in os.listdir(os.path.join(".","models")) if re.match(r'^{}'.format(prefix),f)]
